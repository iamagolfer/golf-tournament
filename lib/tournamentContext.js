...
// The club roster spans both tournaments — the same people, one handicap — so it
// is not any one tournament's to guard. Being signed in to either is enough.
function requireAnyAdmin(req, res, next) {
  const admin = req.session.admin || {};
  if (!SLUGS.some(slug => admin[slug])) {
    return res.status(401).json({ error: 'Unauthorized' });
  }
  next();
}
...
module.exports = {
  RING, GREENJACKET, SLUGS, resolveSlug, getTournament, requireAdmin, requireAnyAdmin,
};
